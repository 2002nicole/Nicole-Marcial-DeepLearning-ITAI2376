...
This folder contains my completed Module 03 lab for **ITAI 2376: Deep Learning**. This was a course lab, not a fully independent project. The notebook included starter code and guided sections provided by the course. My work included completing the required coding sections, organizing the image dataset into train/validation/test folders, building missing CNN layers, training and evaluating models, completing the fine-tuning challenge, and writing reflection/analysis responses.

The purpose of this lab was to practice **Convolutional Neural Networks (CNNs)** for image classification using a small “puppy or bagel” image dataset.
...
The dataset is intentionally small. In my notebook, the dataset contained:
...
The second model used **MobileNetV2**, a pre-trained CNN model. The base MobileNetV2 model was loaded with ImageNet weights and frozen so that its learned visual feature detectors would not be updated during the first training step.
...
MobileNetV2 was selected because it is lightweight and works well in Google Colab.
...
Fine-tuning did not clearly improve performance because the original transfer learning model was already at 100% test accuracy. The test loss increased slightly from 0.0021 to 0.0062, so there was no strong evidence that fine-tuning made the model better in this lab.
...
- Convolutional filters help detect visual patterns such as edges, textures, and shapes.
...
This was a guided course lab with starter code provided as part of ITAI 2376. My contributions included completing the required CNN coding sections, organizing the dataset into a working folder structure, training and evaluating the models, completing the transfer learning fine-tuning challenge, and writing the reflection and analysis responses.
